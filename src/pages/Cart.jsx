...
const Cart = () => {
    ...
    const { products, currency, cartItems, updateCartItemQuantity,updateQuantity, navigate } = useContext(ShopContext)
                    ...
                    cartData.map((item, index) => {
                        ...
                        return (
                            <div key={index} className="grid py-4 border-t border-b text-gray-700 grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4">
                                ...
                                <input
                                    className="border w-10 sm:w-12 h-8 text-center rounded-sm"
                                    type="number"
                                    min={1}
                                    value={item.quantity}
                                    onChange={(e) => e.target.value === '' || e.target.value === '0' ? null : updateCartItemQuantity(item._id, item.size, Number(e.target.value))}
                                />
