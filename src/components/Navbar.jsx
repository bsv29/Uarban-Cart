...
const Navbar = () => {
    ...
    return (
        <div className="flex items-center justify-between px-10 py-5 font-medium w-full">
            <Link to='/'>
                <p className="text-2xl">Urban Cart</p>
            </Link>
            ...
            <div className="flex items-center gap-6">
                ...
                <div className="group relative">
                    ...
                    <div className="group-hover:block hidden absolute dropdown-menu right-0 pt-4">
                        <div className="flex flex-col gap-2 w-40 py-3 px-5 bg-slate-100 shadow-lg rounded-md text-gray-600">
                            {user ? (
                                <>
                                    <p className="text-xs text-gray-500 truncate">{user.email}</p>
                                    <Link className="cursor-pointer hover:text-black" to="/orders">Orders</Link>
                                    <button onClick={logout} className="text-left hover:text-black">Logout</button>
                                </>
                            ) : (
                                <Link className="hover:text-black" to="/login">Login / Register</Link>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <div className = {`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
                <div className="flex flex-col text-grey-600">
                    <div onClick={() => setVisible(false)} className="flex items-center gap-4 p-3 cursor-pointer">
                        <img className="h-4 rotate=180" src={assets.dropdown_icon} alt="dropdwon_icon"/>
                        <p>Back</p>
                    </div>
